...
export const getBranchSettings = createServerFn({ method: "POST" })
  .inputValidator((input: { branchId: string }) =>
    z.object({ branchId: z.string().uuid() }).parse(input),
  )
  .handler(async ({ data }) => {
  ...
  return [
    {
      ...
      enabled: true,
      description: "Earn and redeem loyalty points on checkout.",
    },
    {
      name: "returns-vouchers",
      enabled: true,
      description: "Generate digital refund vouchers automatically.",
    },
    {
      name: "sms-notifications",
      enabled: false,
      description: "Send order status notifications via SMS.",
    },
  ];
});
