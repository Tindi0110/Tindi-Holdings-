...
export const getSimilarProducts = createServerFn({ method: "POST" })
  .inputValidator((input: { categoryId: string; productId: string }) =>
    z.object({ categoryId: z.string().uuid(), productId: z.string().uuid() }).parse(input),
  )
  .handler(async ({ data }) => {
...
export const getNewArrivalRecommendations = createServerFn({ method: "GET" }).handler(async () => {
