...
export class ReportingRepository {
  static async fetchAllMetrics() {
    ...
    const { count: customers } = await supabaseAdmin.from("profiles").select("*", { count: 'exact', head: true });
    const { count: products } = await supabaseAdmin.from("products").select("*", { count: 'exact', head: true });
    const { count: lowStock } = await supabaseAdmin.from("products").select("*", { count: 'exact', head: true }).lt("stock", 10);
    
    const todayStr = new Date().toISOString().slice(0, 10);
    const orderList = orders ?? [];
    
    const totalRevenue = orderList.filter(o => o.status !== 'cancelled').reduce((sum, o) => sum + Number(o.total), 0);
    const totalOrders = orderList.length;
    const todayOrders = orderList.filter(o => o.created_at.slice(0,10) === todayStr).length;
    const todayRevenue = orderList.filter(o => o.created_at.slice(0,10) === todayStr && o.status !== 'cancelled').reduce((sum, o) => sum + Number(o.total), 0);
  ...
  static async fetchDailyRevenue() {
    const { data, error } = await supabaseAdmin.from("orders").select("total, created_at, status").order("created_at");
  ...
  static async fetchBranchRevenue() {
    const { data, error } = await supabaseAdmin.from("orders").select("total, status, branches(name)");
    if (error) throw new Error(error.message);
  ...
  static async fetchTopProducts() {
    const { data, error } = await supabaseAdmin.from("order_items").select("product_name, quantity, unit_price");
    if (error) throw new Error(error.message);
