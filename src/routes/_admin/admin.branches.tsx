...
import {
  Plus, Pencil, Trash2, MapPin, Phone, Sparkles, ArrowRightLeft,
  Truck, CheckCircle, Package, RefreshCw, Clock, Building2,
    ...
    <AdminShell title="Multi-Branch & Logistics">
          ...
          <div className="flex items-center gap-4">
            ...
            <div>
              <div className="flex items-center gap-2">
                <span className="text-[10px] font-black uppercase tracking-widest text-primary">Enterprise Logistics</span>
                <span className="text-[10px] text-muted-foreground">•</span>
                <span className="text-[10px] font-bold text-muted-foreground uppercase">{branches?.length ?? 0} Active Nodes</span>
              </div>
              <h2 className="text-xl font-black uppercase tracking-tight text-foreground mt-0.5">Multi-Branch Node Network</h2>
            </div>
          </div>
            ...
            <div className="flex items-center p-1 bg-muted/40 rounded-xl border border-border">
              ...
              <button
                ...
                className={`px-4 py-2 rounded-lg text-xs font-black uppercase tracking-wider transition-all cursor-pointer flex items-center gap-1.5 ${
                  activeTab === "transfers" ? "bg-primary text-primary-foreground shadow-sm" : "text-muted-foreground hover:text-foreground"
                }`}
              >
                <ArrowRightLeft className="h-3.5 w-3.5" />
                Inter-Branch Transfers ({transfers.filter((t: any) => t.status === "in_transit").length} Live)
              </button>
            </div>
        ...
        {activeTab === "branches" && (
          <div className="space-y-4">
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {(branches ?? []).map((b) => (
                <div key={b.id} className="bg-card border border-border rounded-2xl p-5 shadow-sm space-y-4 hover:border-primary/40 transition-colors">
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <span className={`inline-flex items-center gap-1 text-[9px] font-black uppercase px-2 py-0.5 rounded ${
                          b.is_active ? "bg-emerald-500/10 text-emerald-600 border border-emerald-500/20" : "bg-muted text-muted-foreground"
                        }`}>
                          <span className={`h-1.5 w-1.5 rounded-full ${b.is_active ? "bg-emerald-500" : "bg-muted-foreground"}`} />
                  ...
                  <div className="space-y-2 pt-2 border-t border-border/50 text-xs">
                    <div className="flex items-center gap-2 text-muted-foreground">
                      ...
                      <span className="truncate">{b.address || "Main Commercial Center, Kenya"}</span>
                    </div>
                    <div className="flex items-center gap-2 text-muted-foreground">
          ...
          <div className="space-y-5">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              <div className="bg-card border border-border rounded-2xl p-5 shadow-sm">
                <span className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Total Transfer Records</span>
                <div className="text-2xl font-black text-foreground mt-1">{transfers.length}</div>
                <p className="text-[11px] text-primary font-semibold mt-0.5">Inter-Branch Shipments</p>
              </div>
              <div className="bg-card border border-border rounded-2xl p-5 shadow-sm">
                <span className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">In Transit</span>
                <div className="text-2xl font-black text-amber-600 mt-1">{transfers.filter((t: any) => t.status === "in_transit").length}</div>
                <p className="text-[11px] text-amber-600 font-semibold mt-0.5">Dispatched on Road</p>
              </div>
              <div className="bg-card border border-border rounded-2xl p-5 shadow-sm">
                <span className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Received & Stocked</span>
                <div className="text-2xl font-black text-emerald-600 mt-1">{transfers.filter((t: any) => t.status === "received").length}</div>
                <p className="text-[11px] text-emerald-600 font-semibold mt-0.5">Inventory Reconciled</p>
              </div>
              <div className="bg-card border border-border rounded-2xl p-5 shadow-sm">
                <span className="text-[10px] font-black uppercase tracking-wider text-muted-foreground">Active Couriers</span>
                <div className="text-2xl font-black text-primary mt-1">Speedaf, Fargo, G4S</div>
                <p className="text-[11px] text-muted-foreground font-semibold mt-0.5">Logistics Partners</p>
              </div>
            </div>
                      ...
                      <tr key={t.id} className="hover:bg-muted/10 transition-colors">
                        ...
                        <td className="px-6 py-4">
                          ...
                          <div className="text-[10px] font-mono text-muted-foreground">
                            SKU: {t.sku} • <strong className="text-primary font-bold font-sans">{t.quantity} Units</strong>
                          </div>
                        </td>
                        <td className="px-6 py-4">
                          <div className="text-xs font-bold text-foreground">{t.courier_name || "In-House Logistics"}</div>
                          <span className="font-mono text-[10px] text-muted-foreground">{t.tracking_number}</span>
                        </td>
                        <td className="px-6 py-4">
                          <span className={`text-[9px] font-black uppercase tracking-wider px-2.5 py-1 rounded-lg border ${
                            ...
                            {t.status === "in_transit" ? "⚡ In Transit" : t.status === "received" ? "✓ Received" : "Rejected"}
                          </span>
                        </td>
                        ...
                        <td className="px-6 py-4 text-right">
                          {t.status === "in_transit" ? (
                            <button
                              onClick={() => updateTransferStatusMut.mutate({ id: t.id, status: "received" })}
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent className="max-w-md bg-card border border-border rounded-2xl p-6 shadow-2xl">
          <DialogHeader>
            <DialogTitle className="font-black text-lg uppercase tracking-tight">{form.id ? "Edit Operational Node" : "Provision New Branch Node"}</DialogTitle>
          </DialogHeader>
          <form
            onSubmit={(e) => {
              e.preventDefault();
          ...
          >
            <div className="space-y-1.5">
              <label className="text-xs font-black text-muted-foreground uppercase block">Branch Name *</label>
              <input
                required
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-black text-muted-foreground uppercase block">Physical Address</label>
              <input
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-black text-muted-foreground uppercase block">Contact Phone</label>
              <input
            ...
            <DialogFooter className="pt-3 border-t border-border gap-2">
              <Button type="button" variant="outline" onClick={() => setOpen(false)} className="rounded-xl text-xs font-bold">
                Cancel
              </Button>
              <Button type="submit" disabled={save.isPending} className="rounded-xl bg-primary text-primary-foreground font-black text-xs uppercase tracking-wider px-6">
      ...
      <Dialog open={isTransferModalOpen} onOpenChange={setIsTransferModalOpen}>
        <DialogContent className="max-w-lg bg-card border border-border rounded-2xl p-6 shadow-2xl">
          <DialogHeader>
            <DialogTitle className="font-black text-lg uppercase tracking-tight">Initialize Inter-Branch Stock Transfer</DialogTitle>
            <p className="text-xs text-muted-foreground mt-1">Reallocate catalog inventory between regional nodes with courier waybill tracking.</p>
          </DialogHeader>
          ...
          <form
            onSubmit={(e) => {
              ...
              const selectedProd = products.find((p: any) => p.id === transferForm.product_id) || products[0];
          ...
          >
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-xs font-black text-muted-foreground uppercase block">Origin Branch (From) *</label>
                <select
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-black text-muted-foreground uppercase block">Destination (To) *</label>
                <select
            ...
            <div className="grid grid-cols-3 gap-3">
              <div className="col-span-2 space-y-1.5">
                <label className="text-xs font-black text-muted-foreground uppercase block">Catalog Product SKU *</label>
                <select
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-black text-muted-foreground uppercase block">Transfer Qty *</label>
                <input
                  type="number"
                  min="1"
                  required
            ...
            <div className="grid grid-cols-2 gap-4">
              <div className="space-y-1.5">
                <label className="text-xs font-black text-muted-foreground uppercase block">Transit Courier</label>
                <select
                  value={transferForm.courier_name}
                  onChange={(e) => setTransferForm({ ...transferForm, courier_name: e.target.value })}
                  className="w-full h-11 px-3 rounded-xl border border-border bg-muted/20 text-xs font-bold outline-none"
                >
              ...
              <div className="space-y-1.5">
                <label className="text-xs font-black text-muted-foreground uppercase block">Waybill / Tracking #</label>
                <input
                  value={transferForm.tracking_number}
                  onChange={(e) => setTransferForm({ ...transferForm, tracking_number: e.target.value })}
            ...
            <div className="space-y-1.5">
              <label className="text-xs font-black text-muted-foreground uppercase block">Dispatch Notes</label>
              <input
            ...
            <DialogFooter className="pt-3 border-t border-border gap-2">
              <Button type="button" variant="outline" onClick={() => setIsTransferModalOpen(false)} className="rounded-xl text-xs font-bold">
                Cancel
              </Button>
              <Button
                type="submit"
